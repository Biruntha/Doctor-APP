...
@section("main-body")
    <h1 class="page-heading rounded">Update Problem  - {{ $problem->name }}</h1>
                ...
                <div class="row">
                        <div class="col-12 col-md-4">
                            <div class="mb-3">
                                ...
                                <input class="form-control mx-1" value="{{ old('name', $problem->name) }}" placeholder="Name" type="text" name="name" />
                            </div>
                        </div>
                        <div class="col-12 col-md-4">
                            <div class="mb-3">
                                ...
                                <select class="form-control" name="problem_type" id="problem_type">
                                    ...
                                    @foreach ($problemTypes as $obj)
                                        <option {{old('problem_type') == '' ? ($problem->problem_type == $obj->id ? 'selected' : '') : (old('problem_type') == $obj->id ? 'selected' : '') }} value="{{$obj->id}}">{{$obj->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-12 col-md-4">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control mx-1" placeholder="Description" type="text" name="description">{{ old('description', $problem->description) }}</textarea>
                            </div>
                        </div>
                    </div>
