@extends("layouts.main")

@section("main-body")
<div class="row my-4 mx-2">
    <div class="col-12 col-md-6">
        <div class="mb-4 p-2 bg-white rounded shadow-md vacancy-card">
            <div class="d-flex">
                <div class="img-cont">
                    @if($doctor->image == null)
                    <img alt="doctor-img" src="data:image/png;base64," />
                    @else
                    <img alt="doctor-img" src="{{asset('storage/UserImages/'.$doctor->image)}}" />
                    @endif
                </div>
                <div class="details">
                    <h4>{{ $doctor->name }}</h4>
                    <p>Appointment #{{ $appointment->appointment_number }}</p>
                    <p>{{ $appointment->date == $currentDate ? 'Today' : ($appointment->date == $tomorrowDate ? 'Tomorrow' : $appointment->date)}}, Est. By {{ $appointment->time_from}}</p>
                </div>
            </div>
            <div class="details">
                <h5>Payment Details</h5>
                <p>Total LKR {{ $appointment->fees }}</p>
            </div>
            <div class="details">
                <h5>Patient Details</h5>     
                <p>Patient Name : {{ $patient->patient_name }}</p>
                <p>Age : {{(new DateTime())->diff(new DateTime($patient->patient_dob))->y}}Y</p>
                <p>Gender : {{ $patient->patient_gender }}</p>
            </div>
        </div>
        <div class="mb-4 p-2 bg-white rounded shadow-md vacancy-card">
            <div class="d-flex">
                <div class="details">
                    <h4>Pay by Wallet</h4>
                    <div class="col-12">
                        <div class="mb-3 d-flex">
                            <form title="Pay with Wallet" method="get">
                                <input type="hidden" name="w_appointment_id" id="w_appointment_id" value='{{$appointment->id}}'>
                                <label for="wallet_amount" class="form-label">Balance is {{$wallet_balance}}</label>
                                <input class="form-control mx-1" value="{{($wallet_balance - $appointment->fees) >= 0 ? $appointment->fees : $wallet_balance}}" placeholder="Wallet Amount" type="number" max={{($wallet_balance - $appointment->fees) >= 0 ? $appointment->fees : $wallet_balance}} min=0 name="wallet_amount" id="wallet_amount"/>
                                <button type="button" onclick="payByWallet()" class="btn btn-primary mx-1">Proceed</button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="mb-4 p-2 bg-white rounded shadow-md vacancy-card">
            <div class="d-flex">
                <div class="details">
                    <h4 id="payment_balance">Balance is {{ $card_balance >= 0 ? $card_balance : 0 }}</h4>
                    <div class="form-check">
                        <form action="{{ route('pay-with-card') }}" title="Pay with Card" method="POST">
                            @csrf
                            <input type="hidden" name="appointment_id" id="appointment_id" value='{{$appointment->id}}'>
                            <!-- <input class="form-check-input" type="radio" name="payment" id="payment" checked> -->
                            <label class="form-check-label" for="payment">
                                Pay with Card
                            </label>
                            <button type="submit" class="btn btn-primary mx-1">Proceed</button>
                        </form>
                    </div>
                    <!-- <div class="form-check">
                        <form action="{{ route('pay-with-slip') }}" title="Pay with Bank Slip" method="POST">
                            @csrf
                            <input type="hidden" name="appointment_id" id="appointment_id" value='{{$appointment->id}}'>
                            <input class="form-check-input" type="radio" name="payment" id="payment">
                            <label class="form-check-label" for="payment">
                                Pay with Bank Slip
                            </label>
                            <button type="submit" class="btn btn-primary mx-1">Proceed</button>
                        </form>
                    </div> -->
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
@section("scripts")
<script>
$(document).ready(function() {
    let appointment=@json($appointment);
    let wallet_balance=@json($wallet_balance);
    console.log(appointment);
    console.log(wallet_balance);

    $('#wallet_amount').on('keyup change', function(e) {
        console.log($("#payment_balance").val());

        if ($('#wallet_amount').val() > wallet_balance ) {
            e.preventDefault();
            $('#wallet_amount').val(wallet_balance);
        }
        $("#payment_balance").html('Balance is ' + (appointment['fees'] - $('#wallet_amount').val()));

    });
    
});

</script>  
@endsection
