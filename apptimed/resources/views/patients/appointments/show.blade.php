...
<div class="row mt-1">
    <div class="col-12 col-md-6">
        <a href="{{$appointment->hangout_link}}" target="_blank">
        <div class="mb-4 p-2 bg-white rounded shadow-md vacancy-card">
            <div class="d-flex">
                <div class="img-cont">
                    ...
                    @else
                    <img alt="doctor-img" src="{{asset('storage/UserImages/'.$appointment->image)}}" />
                    @endif
                </div>
                <div class="details">
                    <h4>{{ $appointment->date }}</h4>
                    <p>Appointment #{{ $appointment->appointment_number }} {{ $appointment->start_time }}</p>
                    <p>{{$appointment->patient_name }} {{$appointment->patient_dob }} {{$appointment->patient_gender }}</p>
                    <p>{{ $appointment->fullname }}</p>
                    <p>Total Amount : {{ $appointment->amount }} Status : {{ $appointment->payment_status }}</p>
                </div>
            </div>
            <div class="d-flex my-2">
                <button type="submit" class="btn btn-primary w-100 mx-1"><i class="far fa-arrow-right"></i> Join Meeting</button>
            </div>
        </div>
        </a>
    </div>
